...
echo "Single GPU: ${gpu}"
echo "Training group 0: views ${train_group0_views}"
echo "Training group 1: views ${train_group1_views}"
echo "Evaluation group 0 views: ${eval_group0_views}"
echo "Evaluation group 1 views: ${eval_group1_views}"

echo "Starting single-GPU sequential view-group training..."
echo "Preparing gsplat CUDA extension cache: ${TORCH_EXTENSIONS_DIR}"
CUDA_VISIBLE_DEVICES=${gpu} python -c "from gsplat.cuda._backend import _C; assert _C is not None"
...
echo "Starting group 0 training on GPU ${gpu}"
if ! CUDA_VISIBLE_DEVICES=${gpu} python -u tools/train.py \
    ...
    view_parallel.group_id=0 > "${group0_log}" 2>&1; then
    echo "Training failed: group0"
    echo "Check log: ${group0_log}"
    echo "===== group0 log tail ====="
    tail -n 80 "${group0_log}" || true
    exit 1
fi

echo "Starting group 1 training on GPU ${gpu}"
if ! CUDA_VISIBLE_DEVICES=${gpu} python -u tools/train.py \
    --config_file "${config_file}" \
    --output_root "${output_root}" \
    --project "${project_name}" \
    ...
    data.end_timestep="${end_timestep}" \
    view_parallel.enabled=true \
    view_parallel.group_id=1 > "${group1_log}" 2>&1; then
    echo "Training failed: group1"
    echo "Check log: ${group1_log}"
...
echo "Starting single-GPU sequential view-group evaluation..."
echo "Evaluating group 0 checkpoint: ${group0_ckpt}"
...
echo "Evaluating group 1 checkpoint: ${group1_ckpt}"
CUDA_VISIBLE_DEVICES=${gpu} python tools/eval.py \
...
CUDA_VISIBLE_DEVICES=${gpu} python tools/merge_view_parallel_render.py \
