...
def rotated_rect_iou(poly1_coords, poly2_coords):
    """
    计算两个旋转矩形之间的 IoU。
    Args:
        poly1_coords (np.array): 矩形1的4个角点坐标 (x, y)。形状 (4, 2)。
        poly2_coords (np.array): 矩形2的4个角点坐标 (x, y)。形状 (4, 2)。
    Returns:
        float: 两个旋转矩形的 IoU 值。
    """
    # 将 NumPy 数组转换为 Shapely Polygon 对象
    from shapely.geometry import Polygon
    poly1 = Polygon(sort_points_clockwise(poly1_coords))
    poly2 = Polygon(sort_points_clockwise(poly2_coords))
