...
BaseImageParaLane = collections.namedtuple("Image", ["id", "qvec", "tvec", "name"])
        ...
        while True:
            ...
            if len(line) > 0 and line[0] != "#":
                ...
                images[image_id] = ImageDataParaLane(
                    id=image_id, qvec=qvec, tvec=tvec, name=image_name
                )
    return images
    ...
    with open(path, "r") as fid:
        while True:
            ...
            if len(line) > 0 and line[0] != "#":
                ...
                cameras[camera_channel] = CameraParaLane(
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for _ in range(num_reg_images):
            ...
            while current_char != b"\x00":  # look for the ASCII 0 entry
                ...
                current_char = readNextBytes(fid, 1, "c")[0]
            num_points2D = readNextBytes(fid, num_bytes=8, format_char_sequence="Q")[0]
            x_y_id_s = readNextBytes(
                fid,
...
def readIntrinsicsBinary(path_to_model_file):
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for _ in range(num_cameras):
            ...
            params = readNextBytes(
                fid, num_bytes=8 * num_params, format_char_sequence="d" * num_params
            )
            cameras[camera_id] = Camera3DRealCar(
                id=camera_id,
                model=model_name,
                width=width,
                height=height,
...
def debugProjections(cam_infos, pcd):
    ...
    for cam_info in cam_infos:
        ...
        projections, _ = cv2.projectPoints(
            pcd.points, world_to_camera_r, world_to_camera_t, intrinsic, None
        )
