...
def getInfo(args):
    ...
    for idx, info in enumerate(meta_info["frames"]):
        ...
        time_with_pose[info["log_time_stamp"]] = (
            float(lidar_to_world[0, 3]),
            float(lidar_to_world[1, 3]),
        )
    ...
    if len(time_with_pose) < chunk_max_image:  # The situation where the log is short
        ...
        for curr_min_x in range(min_x, max_x, chunk_size):
            ...
            for curr_min_y in range(min_y, max_y, chunk_size):
                ...
                block_id_with_rect[curr_block_id] = [
                    curr_min_x,
                    curr_min_y,
                    curr_max_x,
                    curr_max_y,
                ...
                block_id_with_extend_rect[curr_block_id] = [
                    curr_min_x - expand_size,
                    curr_min_y - expand_size,
                    curr_max_x + expand_size,
                    curr_max_y + expand_size,
                ]
        ...
        for timestamp, pos in time_with_pose.items():
            ...
            if (
                use_downsample and np.linalg.norm(curr_pos - last_pos) < 0.1
            ):  # TODO This is a problem. When the car is stationary, the dynamic object motion will introduce bugs
                continue
        ...
        for key, values in block_id_with_timelist.items():
            ...
            if (
                use_downsample and len(curr_value) > chunk_max_image * 1.5
            ):  # TODO  掉头转弯？？这种原地操作不应该删掉 而且动态物体有些帧也会被跳掉！
                curr_value.sort()
                interval = max(int(len(curr_value) / chunk_max_image), 2)
                curr_value = [
                    curr_value[i] for i in range(0, len(curr_value), interval)
                ]
        ...
        for timestamp, pos in time_with_pose.items():
            ...
            for block_id, area in block_id_with_rect.items():
                ...
                curr_distance = (pos_x - mid_x) * (pos_x - mid_x) + (pos_y - mid_y) * (
                    pos_y - mid_y
                )
        ...
        block_time_without_extend = OrderedDict(
            sorted(block_time_without_extend.items())
        )

    # save json files
    block_info = {
        "block_time_with_extend": block_time_with_extend,
        "block_time_without_extend": block_time_without_extend,
        "block_id_with_rect": block_id_with_rect,
...
def dataPartitionChery(args, block_size, single_block_test=True):
    ...
    if single_block_test and len(block_time) > 0:
        first_block_id = min(block_time.keys())
        print(
            "[ Info ] single_block_test enabled, only keep block {} ({} frames)".format(
                first_block_id, len(block_time[first_block_id])
            )
        )
        block_time = {first_block_id: block_time[first_block_id]}
    ...
    with open(block_info_json, "w", encoding="utf-8") as file:
        json.dump(block_info, file, ensure_ascii=False, indent=4)
    print(block_info_json)
    print("[ Info ] end dataPartitionSimple")
    return block_time_with_extend, block_time_without_extend
...
def dataPartitionSimple(args, single_block_test=True):
    ...
    if single_block_test:
        block_time = {0: all_timestamp[50:100]}
    else:
        block_time = {
