...
def render_trajectory(
    ...
    for traj_type, traj in render_traj.items():
        ...
        render_data = dataset.prepare_novel_view_render_data(traj)  # traj 为单个相机（前向）的新轨迹
        ...
        if depth_maps is not None and generate_lidar_pc:
            ...
            intrinsics = {cam_id: camera_data[cam_id].intrinsics.cpu().numpy() for cam_id in cam_ids}
            T_cam_to_lidars = {cam_id: camera_data[cam_id].cam_to_main_lidar for cam_id in cam_ids}
...
def main(args):
    ...
    if args.cam_ids is not None:
        ...
        cfg.data.pixel_source.cameras = args.cam_ids
    ...
    if args.generate_lidar_pc and not args.render_depth:
        args.render_depth = True
        print("生成点云需要深度图，已自动开启 render_depth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--render_rgb", type=bool, default=False, help='render rgb novel views')
    parser.add_argument("--render_depth", type=bool, default=False, help='render depth novel views')
    
    parser.add_argument("--generate_lidar_pc", type=bool, default=False, help='generate point cloud for each frame')
    # viewer
