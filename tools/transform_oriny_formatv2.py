...
### 直接使用源数据的pcd文件
### 源数据格式：images_source包含时间戳文件夹000,001,002...，每个文件夹包含0_rgbs.png到12_rgbs.png
import os
import shutil
import glob
...
def process_data():
    # 定义路径
    images_source = "/data4/gls/code/chery_scene_reconstruction/output/qcraft_20250702_133223_Q2517/20251007_lidar+cam0_1_2_3_4_5_6_8_9_10_12/novel_traj/original_traj_step40000/images"
    ...
    # 获取所有时间戳文件夹 (000, 001, 002等)
    timestamp_folders = sorted([f for f in os.listdir(images_source) 
                               if os.path.isdir(os.path.join(images_source, f)) and f.isdigit()])
    
    print(f"找到 {len(timestamp_folders)} 个时间戳文件夹: {timestamp_folders[:5]}...")  # 显示前5个
    
    # 获取raw文件夹中的时间戳文件夹列表（用于映射）
    raw_timestamp_folders = sorted(list(image_timestamp_map.keys()))
    
    # 为每个时间戳创建文件夹并复制/重命名图像
    for timestamp_folder in timestamp_folders:
        timestamp_key = timestamp_folder  # 如 '000', '001' 等
        
        # 建立 processed 中的 000, 001 等与 raw 中时间戳文件夹的映射
        # 按顺序匹配：000 -> 第一个raw时间戳文件夹，001 -> 第二个，依此类推
        if len(raw_timestamp_folders) > int(timestamp_key):
            raw_timestamp_folder = raw_timestamp_folders[int(timestamp_key)]
        else:
        ...
        # 源文件夹路径（包含0_rgbs.png到12_rgbs.png）
        source_timestamp_dir = os.path.join(images_source, timestamp_folder)
        output_timestamp_dir = os.path.join(output_base, raw_timestamp_folder)
        os.makedirs(output_timestamp_dir, exist_ok=True)
        
        print(f"处理时间戳文件夹: {timestamp_folder} -> {raw_timestamp_folder}")
        
        # 复制每个视角的图像 (0-12)
        for camera_idx in range(13):  # 0到12
            source_img = os.path.join(source_timestamp_dir, f"{camera_idx}_rgbs.png")
            ...
            if os.path.exists(source_img):
                if camera_idx in image_timestamp_map[raw_timestamp_folder]:
                    ...
                    print(f"  复制图像: {camera_idx}_rgbs.png -> {new_filename}")
                else:
                    print(f"  警告: 时间戳文件夹 {raw_timestamp_folder} 中未找到相机 {camera_idx} 的时间戳信息")
            else:
                print(f"  警告: 源图像文件不存在 {source_img}")
