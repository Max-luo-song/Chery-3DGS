...
def match_and_postprocess(
            ...
            for i_, tid in enumerate(gt_track_f["extra_data"]["gt_track_id"]):
                if tid not in parsed_gt_track:
                    parsed_gt_track[tid] = {
                        "valids": torch.zeros(num_f, dtype=torch.bool),
                        "tracked_bbox": torch.zeros(num_f, 4),
                        "clipped": torch.zeros(num_f, dtype=torch.bool),
                        "image_height": torch.tensor(gt_track_f["image_height"]),
                        "image_width": torch.tensor(gt_track_f["image_width"]),
                    }
                parsed_gt_track[tid]["valids"][fi] = True
                parsed_gt_track[tid]["tracked_bbox"][fi] = torch.tensor(gt_track_f["gt_bbox"][i_])
                parsed_gt_track[tid]["clipped"][fi] = torch.tensor(gt_track_f["clipped"][i_])
    ...
    for gt_tid in valid_gt_tids:
        collector[gt_tid] = {
            ...
            "box_on_boundary": torch.zeros(num_c, num_f).bool(),
            ...
            for gt_tid in valid_gt_tids:
                ...
                for fi in range(num_f):
                    if parsed_gt_track[gt_tid]["valids"][fi]:
                        collector[gt_tid]["2DBox_appear_mask"][idx, fi] = 1
                        box = parsed_gt_track[gt_tid]["tracked_bbox"][fi]
                        collector[gt_tid]["area"][idx, fi] = box[2] * box[3]
                        collector[gt_tid]["box_on_boundary"][idx, fi] = parsed_gt_track[gt_tid]["clipped"][fi]
            ...
            for gt_tid in valid_gt_tids:
                ...
                for fi in range(num_f):
                    if parsed_gt_track[gt_tid]["valids"][fi] and parsed_pred_track[matched_pred_tid]["valids"][fi]:
                        collector[gt_tid]["smpl"]["global_orient"][idx, fi] = parsed_pred_track[matched_pred_tid]["smpl"]["global_orient"][fi].clone()
                        collector[gt_tid]["smpl"]["body_pose"][idx, fi] = parsed_pred_track[matched_pred_tid]["smpl"]["body_pose"][fi].clone()
                        collector[gt_tid]["smpl"]["betas"][idx, fi] = parsed_pred_track[matched_pred_tid]["smpl"]["betas"][fi].clone()
                        collector[gt_tid]["camera"][idx, fi] = parsed_pred_track[matched_pred_tid]["camera"][fi].clone()
                        collector[gt_tid]["matched_mask"][idx, fi] = 1
    ...
    if save_temp:
        for idx, cam_id in enumerate(camera_list):
            ...
            for fi in range(num_f):
                ...
                for gt_tid in valid_gt_tids:
                    if collector[gt_tid]["valid_mask"][idx, fi]:
                        fi_info["tid"].append(gt_tid)
                        fi_info["tracked_time"].append(0)
                        fi_info["smpl"].append({
                            "global_orient": collector[gt_tid]["smpl"]["global_orient"][idx, fi].numpy(),
                            "body_pose": collector[gt_tid]["smpl"]["body_pose"][idx, fi].numpy(),
                            "betas": collector[gt_tid]["smpl"]["betas"][idx, fi].numpy(),
                        })
                        fi_info["camera"].append(collector[gt_tid]["camera"][idx, fi].numpy())
                pkl_dict[fi] = fi_info
            joblib.dump(
    ...
    for gt_tid in valid_gt_tids:
        merged_collector[gt_tid] = {
            ...
            "camera": torch.zeros(num_c, num_f, 3),                 # 3 -> num_c
            "2DBox_appear_mask": torch.zeros(num_c, num_f).bool(),  # 3 -> num_c
        ...
        # if matched_sum > 1 means multiple cams get the smpl, we select the smpl from the cam with the best quality
        # Quality criteria (in order):
        #   1. Prefer boxes NOT on image boundary (not touching edges)
        #   2. Among boxes with same boundary status, prefer larger area
        area = collector[gt_tid]["area"]
        box_on_boundary = collector[gt_tid]["box_on_boundary"]
        # Sort by: 1) box_on_boundary (False < True), 2) area (larger is better)
        # Create a score: boundary penalty (0 or 1) + normalized area
        max_area = area.max(dim=0, keepdim=True)[0]
        normalized_area = area / (max_area + 1e-6)  # Normalize to [0, 1]
        boundary_penalty = box_on_boundary.float()  # False=0, True=1
        score = -boundary_penalty + normalized_area  # Prefer not on boundary, then larger area
        max_cam_id = score.argmax(0)
    ...
    for gt_tid in valid_gt_tids:
        ...
        if need_fill.any():
            ...
            mask = merged_collector[gt_tid]["matched_mask"][final_complete_mask]                # (num_f)
            ...
            for id in range(merged_collector[gt_tid]["camera"].shape[0]):                       # (num_c, num_f, 3)
                feature_masked = merged_collector[gt_tid]["camera"][id, final_complete_mask]    # (num_f, 3)
                feature_full = interpolate_features(mask, feature_masked)
                merged_collector[gt_tid]["camera"][id, final_complete_mask] = feature_full
    ...
    for gt_tid in valid_gt_tids:
        ...
        if need_fill.any():
            ...
            mask = merged_collector[gt_tid]["must_appear_mask"][final_complete_mask]            # (num_f)
    ...
    # pop out 2DBox_appear_mask and camera, selected_cam_idx map 0~num_c-1 to camera_list[0~num_c-1]
    for gt_tid in valid_gt_tids:
        merged_collector[gt_tid].pop("2DBox_appear_mask")
        merged_collector[gt_tid].pop("camera")
        # Convert camera position index to actual camera ID
        # Keep -1 unchanged, only map 0~num_c-1 to camera_list[0~num_c-1]
        selected_cam_idx = merged_collector[gt_tid]["selected_cam_idx"]
        valid_mask = merged_collector[gt_tid]["valid_mask"]
        if valid_mask.any():
            selected_cam_idx[valid_mask] = torch.tensor(
                [camera_list[idx.item()] for idx in selected_cam_idx[valid_mask]],
                dtype=torch.long
            )
