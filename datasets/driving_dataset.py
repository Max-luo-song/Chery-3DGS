...
class DrivingDataset(SceneDataset):
    def __init__(
        ...
        if self.lidar_source is not None:  # gls
            # Compute the scene AABB before view-specific LiDAR visibility filtering.
            # In view-parallel mode, one camera group can see no LiDAR points, but both
            # groups still need the same scene-level bounds.
    ...
    def filter_pts_in_road(
        ...
        if TYPE == "IMAGE":
            ...
            if lidar_road_pts.shape[0] == 0:
                filtered_road_pts = lidar_road_pts
                filtered_road_colors = lidar_road_colors
                filtered_road_time = lidar_road_time
                print("No LIDAR ground points available; skipping image road-mask filtering.")
            ...
            on_road_mask = torch.zeros_like(lidar_road_pts[:, 0]).bool()
            ...
            for cam_id in self.pixel_source.camera_list:
                ...
                for frame_idx in range(num_frames):
                    ...
                    if final_valid_mask.any():
                        ...
                        is_on_road_for_frame = current_road_mask[valid_pixels[:, 1], valid_pixels[:, 0]]
                        current_ego_mask_float = (1.0 - current_ego_mask).float()
                        ego_mask_at_valid_pixels = current_ego_mask_float[valid_pixels[:, 1], valid_pixels[:, 0]]
                        is_on_road_for_frame_wo_ego = is_on_road_for_frame * ego_mask_at_valid_pixels
    ...
    def filter_pts_in_road_lidar(
        ...
        if seed_pts.shape[0] == 0:
            empty_pts = seed_pts
            empty_colors = seed_colors
            empty_time = seed_time
            print("No LIDAR points available for PMF ground segmentation; returning empty road/environment points.")
            return empty_pts, empty_colors, empty_time, empty_pts, empty_colors, empty_time
    ...
    def project_lidar_pts_on_images(self, delete_out_of_view_points: bool = True):
        ...
        if self.depth_mode == DepthMode.MULTI_FRAME_STATIC:
            if len(static_lidar_points) == 0:
                logger.warning("No static LiDAR points are visible in the selected cameras; using empty dense depth maps.")
                for cam in self.pixel_source.camera_data.values():
                    dense_depth_maps = [
                        torch.zeros(cam.HEIGHT, cam.WIDTH, device=self.device)
                        for _ in range(len(cam))
                    ]
