...
class DrivingDataset(SceneDataset):
    def __init__(
        ...
        if self.lidar_source != None:
            self.project_lidar_pts_on_images( # gls
    ...
    def get_init_objects(
        ...
        for fi in range(self.frame_num):
            if self.lidar_source != None:
                ...
                lidar_dict = None
                lidar_pts = np.random.rand(10000, 3).astype(np.float64)
            ...
            for ins_id in range(self.instance_num):
                ...
                if self.lidar_source != None:
                    ...
                    valid_colors = torch.from_numpy(np.random.rand(valid_pts.shape[0], 3).astype(np.float64)).to(self.device)
