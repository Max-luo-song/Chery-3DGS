...
class SceneLidarSource(abc.ABC):
    ...
    def get_aabb(self) -> Tensor:
        ...
        if len(lidar_pts) == 0:
            raise RuntimeError("Cannot compute LiDAR AABB because no LiDAR points were loaded.")

        # downsample the lidar points by uniformly sampling a subset of them
        sample_count = max(1, int(len(lidar_pts) / self.data_cfg.lidar_downsample_factor))
        lidar_pts = lidar_pts[torch.randperm(len(lidar_pts), device=lidar_pts.device)[:sample_count]]
