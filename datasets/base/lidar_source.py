...
class SceneLidarSource(abc.ABC):
    ...
    def get_aabb(self) -> Tensor:
        ...
        random_indices = torch.randperm(len(lidar_pts))
        keep_num = int(len(lidar_pts) / self.data_cfg.lidar_downsample_factor)
        QUANTILE_SAFE_THRESHOLD = 2 ** 24
        keep_num = min(keep_num, QUANTILE_SAFE_THRESHOLD -1)
        lidar_pts = lidar_pts[random_indices[:keep_num]]
