...
class CameraData(object):
    def __init__(
        ...
        self.device = device
        self.render_only = render_only  # syc
    ...
    def load_calibrations(
        ...
        # self.intrinsics_scaled = intrinsics_scaled  # (num_frames, 3, 3) | None
            ...
            if self.dataset_name == "qcraft":
                if self.weather_type is not None:
                    img_filepaths.append(
                        os.path.join(self.data_path, f"images_{self.weather_type}", f"{t:06d}_{self.cam_id}.png")
                    )
                else:
                    img_filepaths.append(
                ...
                road_mask_filepaths.append(
                    os.path.join(self.data_path, "road_masks", f"{t:06d}_{self.cam_id}.png")
                )
            else:
                img_filepaths.append(
    ...
    def load_egocar_mask(self):
        ...
        if os.path.exists(egocar_mask):
            ...
            self.egocar_mask = torch.zeros(self.load_size, dtype=torch.bool)
            logger.info(f"No ego car mask found for camera {self.cam_id}.")

    def load_ego_template(self):
        """
        加载ego区域的像素模板（用于novel view渲染）
        由于ego车身相对相机是静态的，只需加载一帧作为模板即可
        """
        from PIL import Image
        import cv2
        
        if len(self.img_filepaths) == 0:
            logger.warning(f"No image files available for ego template, camera {self.cam_id}")
            self.ego_template = None
            return
        
        # 使用第0帧作为ego模板
        template_frame_idx = 0
        img_path = self.img_filepaths[template_frame_idx]
        
        if not os.path.exists(img_path):
            logger.warning(f"Ego template image not found: {img_path}")
            self.ego_template = None
            return
        ...
        # resize到加载尺寸
        img = img.resize((self.load_size[1], self.load_size[0]), Image.BILINEAR)
        img = np.array(img)
        
        # 如果需要去畸变（与训练时保持一致）
        if self.undistort and self.intrinsics is not None:
            img = cv2.undistort(
                img,
                self.intrinsics[0].cpu().numpy(),
                self.distortions[0].cpu().numpy(),
            )
        
        # 归一化到 [0, 1] 并转为tensor
        img_tensor = torch.from_numpy(img).float() / 255.0
        
        # 只保留ego区域的像素（其他区域会被mask掉，这里预先相乘可以节省后续计算）
        # ego_mask: [H, W], img_tensor: [H, W, 3]
        self.ego_template = img_tensor * self.egocar_mask[..., None]
        
        logger.info(f"Loaded ego template from frame {template_frame_idx} for camera {self.cam_id}")
    ...
    def load_road_masks(self):
        ...
        try:
            ...
            raise e        
        self.road_masks = torch.from_numpy(np.stack(road_masks, axis=0))
    ...
    def to(self, device: torch.device):
        ...
        # 2. Ego 模板数据
        # ==========================================================
        if hasattr(self, 'egocar_mask') and self.egocar_mask is not None:
            self.egocar_mask = self.egocar_mask.to(device)
        if hasattr(self, 'ego_template') and self.ego_template is not None:
            self.ego_template = self.ego_template.to(device)

        # ==========================================================
        # 3. 训练/验证专有数据 (仅在非 render_only 模式下加载)
        # ==========================================================
        if not self.render_only:
            self.images = self.images.to(device)
                
            if hasattr(self, 'dynamic_masks') and self.dynamic_masks is not None:
                self.dynamic_masks = self.dynamic_masks.to(device)
                
            if hasattr(self, 'human_masks') and self.human_masks is not None:
                self.human_masks = self.human_masks.to(device)
                
            if hasattr(self, 'vehicle_masks') and self.vehicle_masks is not None:
            ...
            if hasattr(self, 'road_masks') and self.road_masks is not None:
                self.road_masks = self.road_masks.to(device)
                
            if hasattr(self, 'lidar_depth_maps') and self.lidar_depth_maps is not None:
                self.lidar_depth_maps = self.lidar_depth_maps.to(device)
                
            if hasattr(self, 'image_error_maps') and self.image_error_maps is not None:
    ...
    def get_image(self, frame_idx: int) -> Tuple[Dict, Dict]:
        ...
        """
        rgb, sky_mask, road_mask = None, None, None
        ...
        if self.downscale_factor != 1.0:
            egocar_mask = (
                torch.nn.functional.interpolate(
                    egocar_mask.float().unsqueeze(0).unsqueeze(0),
                    scale_factor=self.downscale_factor,
                    mode="nearest",
                )
                .squeeze(0)
        ...
        if self.sky_masks is not None:
            ...
            if self.downscale_factor != 1.0:
                sky_mask = (
                    torch.nn.functional.interpolate(
                        sky_mask.float().unsqueeze(0).unsqueeze(0),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.road_masks is not None:
            ...
            if self.downscale_factor != 1.0:
                road_mask = (
                    torch.nn.functional.interpolate(
                        road_mask.float().unsqueeze(0).unsqueeze(0),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.dynamic_masks is not None:
            ...
            if self.downscale_factor != 1.0:
                dynamic_mask = (
                    torch.nn.functional.interpolate(
                        dynamic_mask.float().unsqueeze(0).unsqueeze(0),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.human_masks is not None:
            ...
            if self.downscale_factor != 1.0:
                human_mask = (
                    torch.nn.functional.interpolate(
                        human_mask.float().unsqueeze(0).unsqueeze(0),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
        ...
        if self.vehicle_masks is not None:
            ...
            if self.downscale_factor != 1.0:
                vehicle_mask = (
                    torch.nn.functional.interpolate(
                        vehicle_mask.float().unsqueeze(0).unsqueeze(0),
                        scale_factor=self.downscale_factor,
                        mode="nearest",
                    )
                    .squeeze(0)
...
class ScenePixelSource(abc.ABC):
    ...
    # (num_frame, )
    per_frame_road_mask: Tensor = None
    ...
    def front_camera_trajectory(self) -> Tensor:
        ...
        """
        cam_idx = 0 if 0 in self.camera_list else self.camera_list[0]  # syc
        front_camera = self.camera_data[cam_idx]
        assert (
    ...
    def _build_novel_view_render_data(
        ...
        if is_original_traj:
            ...
            print("NOVEL TRAJECTORY")

        intrinsics = camera_data.intrinsics[0]  # Assume intrinsics are constant across frames
        kb_coeffs = camera_data.distortions[0]  # Assume distortions are constant across frames

        H, W = camera_data.HEIGHT, camera_data.WIDTH
        N = len(traj)

        normed_time_linspace = torch.linspace(0, 1, len(traj))

        x, y = torch.meshgrid(torch.arange(W, device=self.device), 
        ...
        h_tensor = torch.tensor([H], dtype=torch.long, device=self.device)
        w_tensor = torch.tensor([W], dtype=torch.long, device=self.device)
        ...
        for i in range(N):
            ...
            origins, viewdirs, direction_norm = get_rays(x_flat, y_flat, c2w, intrinsics)
            ...
            ego_mask = camera_data.egocar_mask
            if is_original_traj:
                # 1. 映射回当前帧在训练时注册的真正绝对 img_idx
                img_idx_val = camera_data.unique_img_idx[i]
                img_idx_tensor = torch.full((H, W), img_idx_val, dtype=torch.long, device=self.device)
                
                # 2. 映射回当前帧真正的归一化时间戳
                normed_time_val = camera_data.normalized_time[i]
                normed_time_tensor = torch.tensor([normed_time_val], dtype=torch.float32, device=self.device)
                
            else:
                img_idx_tensor = torch.full((H, W), i, dtype=torch.long, device=self.device)
                normed_time_tensor = torch.tensor([normed_time_linspace[i]], dtype=torch.float32, device=self.device)
            ...
            image_infos = {
                ...
                "viewdirs": viewdirs,
                "direction_norm": direction_norm,
                "img_idx": img_idx_tensor,
                "normed_time": normed_time_tensor,
                "pixel_coords": pixel_coords,
                "egocar_masks": ego_mask,
            }
            image_infos = {k: v for k, v in image_infos.items() if v is not None}
    ...
    def _build_online_render_data(
        ...
        for i in range(N):
            ...
            image_infos = {
                ...
                "normed_time": torch.tensor([normed_time], device=self.device),
                "pixel_coords": pixel_coords,
                "egocar_masks": camera_data.egocar_mask,  # ego车身mask
            }
            image_infos = {k: v for k, v in image_infos.items() if v is not None}
    ...
    def prepare_novel_view_render_data(
        self,
        dataset_type: str,
        ego_novel_traj: torch.Tensor,
        target_cam_data: CameraData,
        is_original_traj: bool = False,
    ) -> list:
        device = ego_novel_traj.device

        # 提取目标相机的外参: T_cam_to_ego
        if hasattr(target_cam_data, 'cam_to_ego') and target_cam_data.cam_to_ego is not None:
            T_camX_to_ego = torch.tensor(target_cam_data.cam_to_ego, device=device, dtype=torch.float32)
        else:
            T_camX_to_ego = torch.eye(4, device=device, dtype=torch.float32)

        camX_traj = []
        for T_ego_to_world in ego_novel_traj:
            # 利用相机的外参，算回相机的世界位姿
            T_camX_to_world = T_ego_to_world @ T_camX_to_ego
