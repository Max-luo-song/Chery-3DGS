...
def run_difix_distill_one_traj(
    ...
    ref_cam_id = args.distill_ref_cam_id
...
def render_trajectory(
    ...
    for traj_type in traj_types:
        ...
        ref_cam_id = (
            int(args.distill_ref_cam_id)
            if args is not None and hasattr(args, "distill_ref_cam_id")
            else 0
        )
        render_traj = dataset.get_novel_render_traj(
            traj_type=traj_type,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="limit number of distilled frames per camera; -1 means all",
    )
    parser.add_argument(
        "--distill_steps",
        type=int,
