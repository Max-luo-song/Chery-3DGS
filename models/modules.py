...
from bilateral.lib_bilagrid import BilateralGrid, slice_grid, total_variation_loss
...
def _apply_affine(rgb: Tensor, affine: Tensor) -> Tensor:
    affine = affine.reshape(*rgb.shape[:-1], 3, 4)
    return (affine[..., :3, :3] @ rgb[..., None] + affine[..., :3, 3:])[..., 0]


class MultiScaleBilateralAffineTransform(nn.Module):
    """BDGS multi-scale per-image bilateral appearance correction."""

    def __init__(self, class_name: str, n: int, grid, device: torch.device = torch.device("cuda")):
        super().__init__()
        self.class_prefix = class_name + "#"
        self.device = device
        self.in_test_set = False
        self.grid_sizes = [tuple(size) for size in grid]
        self.bil_grids = nn.ModuleList(
            [BilateralGrid(n, grid_X=size[0], grid_Y=size[1], grid_W=size[2]) for size in self.grid_sizes]
        )
        self.tv_weights = [0.5 * (size[0] * size[1] * size[2]) ** 0.5 for size in self.grid_sizes]
        self.guidance_factors = [4] * max(len(self.grid_sizes) - 1, 0) + [2]
        self._last_affines = None

    def _sample_affine(self, bil_grid: BilateralGrid, rgb: Tensor, img_idx: Tensor, factor: int) -> Tensor:
        height, width = rgb.shape[:2]
        sample_size = (max(1, height // factor), max(1, width // factor))
        sampled_rgb = F.interpolate(
        ...
        grid_y, grid_x = torch.meshgrid(
            torch.linspace(0.0, 1.0, sample_size[0], device=rgb.device),
            torch.linspace(0.0, 1.0, sample_size[1], device=rgb.device),
            indexing="ij",
        )
        xy = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(0)
        index = None if self.in_test_set else img_idx.flatten()[:1].long()
        affine = slice_grid(bil_grid, xy, sampled_rgb, index)
        if sample_size != (height, width):
            affine = F.interpolate(
                affine.flatten(start_dim=-2).permute(0, 3, 1, 2),
                size=(height, width),
                mode="bilinear",
                align_corners=False,
            ).permute(0, 2, 3, 1).reshape(1, height, width, 3, 4)
        return affine.squeeze(0)

    def forward(self, rgb: Tensor, image_infos):
        assert "img_idx" in image_infos
        affines = [
            self._sample_affine(grid, rgb, image_infos["img_idx"], factor)
            for grid, factor in zip(self.bil_grids, self.guidance_factors)
        ]
        self._last_affines = affines
        return affines

    def tv_loss(self) -> Tensor:
        return sum(
            weight * total_variation_loss(grid.grids)
            for grid, weight in zip(self.bil_grids, self.tv_weights)
        )

    def inverse_loss(self, gt: Tensor, render: Tensor) -> Tensor:
        if self._last_affines is None:
            return render.new_zeros(())
        transformed = gt
        for affine in reversed(self._last_affines):
            transform = torch.eye(4, device=gt.device, dtype=gt.dtype).expand(*gt.shape[:-1], 4, 4).clone()
            transform[..., :3, :] = affine
            inverse = torch.linalg.inv(transform)[..., :3, :]
            transformed = _apply_affine(transformed, inverse)
        return torch.abs(transformed - render).mean()

    def get_param_groups(self):
        return {
            f"{self.class_prefix}grid{i}": grid.parameters()
            for i, grid in enumerate(self.bil_grids)
        }
