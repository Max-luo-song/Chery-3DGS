...
def reduce_per_gaussian(values: torch.Tensor, n_gaussians: int, reduce: str = "amax") -> torch.Tensor:
    """
    Reduce a tensor with extra gsplat dimensions back to one scalar per gaussian.

    Older gsplat versions typically returned shape [N], while newer versions may
    return shapes like [N, K] or [K, N]. This helper finds the gaussian axis and
    reduces all other axes.
    """
    if n_gaussians == 0:
        return values.reshape(-1)[:0]

    # Some node types (for example SMPLNodes) deliberately return no gaussians
    # when none of their instances are visible in the current frame.  They are
    # therefore absent from pts_labels, but their filter_mask still describes
    # the model's full point set.  Treat that case as "all invisible" so both
    # visibility losses and post-training statistics keep their expected shape.
    if values.numel() == 0:
        return values.new_zeros(n_gaussians)

    if values.ndim == 1:
        if values.shape[0] != n_gaussians:
            raise RuntimeError(
                f"Expected {n_gaussians} gaussian values, but got shape {tuple(values.shape)}."
            )
        return values

    gaussian_dims = [dim for dim, size in enumerate(values.shape) if size == n_gaussians]
    if not gaussian_dims:
        raise RuntimeError(
            f"Could not find gaussian dimension of size {n_gaussians} in tensor shape {tuple(values.shape)}."
        )

    gaussian_dim = gaussian_dims[0]
    values = values.movedim(gaussian_dim, 0).reshape(n_gaussians, -1)
