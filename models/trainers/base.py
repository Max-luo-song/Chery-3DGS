...
class BasicTrainer(nn.Module):
    ...
    def postprocess_per_train_step(self, step: int) -> None:
        ...
        for class_name in self.gaussian_classes.keys():
            ...
            class_radii, class_grads = self._get_class_radii_and_grads(
                radii=radii,
                grads=grads,
    ...
    def update_visibility_filter(self) -> None:
        for class_name in self.gaussian_classes.keys():
            gaussian_mask = self.pts_labels == self.gaussian_classes[class_name]
            self.models[class_name].cur_radii = self._get_class_radii(
                radii=self.info["radii"],
                gaussian_mask=gaussian_mask,
            )

    def _move_gaussian_dim_front(self, tensor: torch.Tensor, num_gaussians: int) -> torch.Tensor:
        tensor = tensor[0] if tensor.shape[0] == 1 else tensor
        shape = tensor.shape[:-1] if tensor.shape[-1] == 2 else tensor.shape
        gaussian_dims = [idx for idx, size in enumerate(shape) if size == num_gaussians]
        if not gaussian_dims:
            raise RuntimeError(
                f"Cannot find gaussian dimension of size {num_gaussians} in tensor shape {tuple(tensor.shape)}"
            )
        return tensor.movedim(gaussian_dims[-1], 0)

    def _get_class_radii(self, radii: torch.Tensor, gaussian_mask: torch.Tensor) -> torch.Tensor:
        radii = self._move_gaussian_dim_front(radii, gaussian_mask.shape[0])[gaussian_mask]
        if radii.shape[0] == 0:
            return radii.reshape(0)
        if radii.dim() == 1:
            return radii
        return radii.reshape(radii.shape[0], -1).max(dim=1).values

    def _get_class_radii_and_grads(
        self,
        radii: torch.Tensor,
        grads: torch.Tensor,
        ...
        radii = self._move_gaussian_dim_front(radii, gaussian_mask.shape[0])[gaussian_mask]
        grads = self._move_gaussian_dim_front(grads, gaussian_mask.shape[0])[gaussian_mask]
        if radii.shape[0] == 0:
            return radii.reshape(0), grads.reshape(0, 2)
        if radii.dim() == 1:
            return radii, grads

        radii = radii.reshape(radii.shape[0], -1)
        grads = grads.reshape(grads.shape[0], -1, 2)
        grad_norms = grads.norm(dim=-1)
        visible = radii > 0
        grad_norms = grad_norms.masked_fill(~visible, -1)
        best_view = grad_norms.argmax(dim=1)
        best_grads = grads[torch.arange(grads.shape[0], device=grads.device), best_view]
        return radii.max(dim=1).values, best_grads
    ...
    def color_legend(self, 
        ...
        if getattr(legend_cfg, "rigid", False):
            self.models["RigidNodes"].color_legned_gaussians(image_output_pth=os.path.join(image_output_pth, "legend_rigid.png"))

        if getattr(legend_cfg, "smpl", False):
            self.models["SMPLNodes"].color_legned_gaussians(image_output_pth=os.path.join(image_output_pth, "legend_smpl.png"))
    ...
    def edit_gaussians(
        ...
        for class_name in edit_cfg.Nodes:
            ...
            # ===== 1. 简单轨迹编辑（无插值）=====
            if "trajectory" in node_cfg.keys():
                traj_cfg = node_cfg.trajectory
                assert len(traj_cfg.instance_id) == len(traj_cfg.offset)
                for instance_id, offset in zip(traj_cfg.instance_id, traj_cfg.offset):
            ...
            # ===== 2. 插值轨迹编辑 =====
            if "trajectory_interp" in node_cfg.keys():
                cfg = node_cfg.trajectory_interp
                instance_ids = _normalize_list(cfg.instance_id, 1, "instance_id")
                num = len(instance_ids)
                waypoints_list = _parse_waypoints(cfg.waypoints, num, "instance_id")
                methods = _normalize_list(cfg.method, num, "method")
                
                for instance_id, waypoints, method in zip(instance_ids, waypoints_list, methods):
            ...
            if "replace" in node_cfg.keys():
                ...
                for target_id, replace_obj in zip(rep_cfg.target_id, rep_cfg.replace_obj):
                    if isinstance(replace_obj, str):
                        ...
                        raise ValueError("replace_obj 只能是 int 或者 str")
            ...
            if "add" in node_cfg.keys():
                ...
                if class_name == "Background":
                    for add_obj, offset in zip(add_cfg.add_obj, add_cfg.offset):
                    ...
                    assert len(ref_ids) == len(add_objs) == len(offsets), \
                        f"长度不匹配: ref_id({len(ref_ids)}), add_obj({len(add_objs)}), offset({len(offsets)})"
                    ...
                    for ref_id, add_obj, offset, waypoints, method in zip(
                        ref_ids, add_objs, offsets, waypoints_list, methods
                    ):
                        if isinstance(add_obj, str):
                            ...
                            raise ValueError("add_obj 只能是 int 或者 str")
    ...
    def affine_transformation(
        ...
        if "Affine" not in self.models:
            return rgb_blended

        affine_model = self.models["Affine"]
        if self.model_config.Affine.type == "models.modules.MultiScaleBilateralAffineTransform":
            rgb_transformed = rgb_blended
            for affine_trs in affine_model(rgb_blended, image_infos):
                rgb_transformed = (
                    affine_trs[..., :3, :3] @ rgb_transformed[..., None]
                    + affine_trs[..., :3, 3:]
                )[..., 0]
            return rgb_transformed

        affine_trs = affine_model(image_infos)
        return (affine_trs[..., :3, :3] @ rgb_blended[..., None] + affine_trs[..., :3, 3:])[..., 0]
    ...
    def forward(
        ...
        outputs["original_rgb"] = outputs["rgb_gaussians"] + outputs["rgb_sky"] * (1.0 - outputs["opacity"])
        outputs["rgb"] = self.affine_transformation(outputs["original_rgb"], image_infos)
    ...
    def compute_losses(
        ...
        if self.depth_loss_fn is not None:
            ...
            if self.depth_loss_fn.mask_out_dynamic_regions:
                lidar_hit_mask = lidar_hit_mask * (1.0 - image_infos["dynamic_masks"])
        ...
        if affine_reg is not None and "Affine" in self.models:
            ...
            if self.model_config.Affine.type == "models.modules.MultiScaleBilateralAffineTransform":
                ...
                target_rgb = image_infos["pixels"] * valid_loss_mask[..., None]
                loss_affine = affine_model.tv_loss()
                loss_cycle = affine_model.inverse_loss(target_rgb, original_rgb)
                loss_dict["affine_loss"] = affine_reg.w * loss_affine + affine_reg.get("w1", 0.0) * loss_cycle
            else:
                affine_trs = affine_model({"img_idx": image_infos["img_idx"].flatten()[0]})
                ...
                loss_dict["affine_loss"] = affine_reg.w * loss_affine
