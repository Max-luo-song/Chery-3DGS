...
class SMPLNodes(RigidNodes):
        ...
        if weight_names or joint_element is not None:
            ...
            if self.use_voxel_deformer:
                ...
                if has_voxel_template:
                    expected_lbs_shape = tuple(self.template.voxel_deformer.lbs_voxel_base.shape[1:])
                    expected_corr_shape = tuple(self.template.voxel_deformer.voxel_w_correction.shape[1:])
                    ...
                    required_meta_fields = ("offset_0", "offset_1", "offset_2", "scale_0")
    ...
    def _build_motion_asset_from_dict(
        ...
        if sparse:
            ...
            motion = {
                ...
                "valid_frame_idx": valid_frame_idx,
        ...
        asset = {"motion": motion}
        if include_template:
            asset["smpl_template"] = self._build_template_asset_from_dict(instance_dict, cpu=cpu)
        return asset
    ...
    def _resolve_reference_source(
        ...
        ref_instances_fv = motion["instances_fv"].to(device=self.instances_fv.device, dtype=self.instances_fv.dtype)
        ref_instances_trans = motion["instances_trans"].to(device=self.instances_trans.device, dtype=self.instances_trans.dtype)
        ref_instances_quats = motion["instances_quats"].to(device=self.instances_quats.device, dtype=self.instances_quats.dtype)

        if ref_instances_trans.ndim != 2 or ref_instances_trans.shape[-1] != 3:
            raise RuntimeError(f"Invalid reference instances_trans shape: {ref_instances_trans.shape}")
        if ref_instances_quats.ndim == 2 and ref_instances_quats.shape[-1] == 4:
            ref_instances_quats = ref_instances_quats.unsqueeze(1).unsqueeze(1)
        elif ref_instances_quats.ndim == 3 and ref_instances_quats.shape[1:] == self.instances_quats.shape[2:]:
            ref_instances_quats = ref_instances_quats.unsqueeze(1)
        if ref_instances_quats.ndim != 4 or ref_instances_quats.shape[2:] != self.instances_quats.shape[2:]:
            raise RuntimeError(
                f"Invalid reference instances_quats shape: {ref_instances_quats.shape}, "
                f"expected (*, 1, {self.instances_quats.shape[2:]})"
            )

        ref_instances_fv = self._resize_reference_mask(ref_instances_fv, self.num_frames).unsqueeze(1)
        ref_instances_trans = self._resize_reference_value(ref_instances_trans, self.num_frames).unsqueeze(1)
        ref_instances_quats = self._resize_reference_value(ref_instances_quats, self.num_frames, identity_quat=True)
        return ref_instances_fv, ref_instances_trans, ref_instances_quats, None, ref_source

    def _normalize_motion_source(
        self,
        motion_source: Optional[Union[int, str, bool]],
    ) -> Optional[Union[int, str]]:
        if motion_source is None:
            return None
        if isinstance(motion_source, bool):
            return None if motion_source is False else None
        if isinstance(motion_source, str):
            stripped = motion_source.strip()
            if stripped.lower() in {"", "none", "null", "keep", "false"}:
                return None
            return stripped
    ...
    def _decode_motion_pose_sequence(
        ...
        if "valid_frame_idx" not in motion or "smpl_qauts_valid" not in motion:
            raise RuntimeError(
                "Unsupported motion format: require either dense "
                "('smpl_qauts') or sparse "
                "('valid_frame_idx'+'smpl_qauts_valid')."
            )

        valid_frame_idx = motion["valid_frame_idx"].to(device=self.device, dtype=torch.long)
        smpl_qauts_valid = motion["smpl_qauts_valid"].to(
            device=self.smpl_qauts.device,
            dtype=self.smpl_qauts.dtype,
        )

        source_frames = motion_fv.shape[0]
        motion_smpl_qauts = torch.zeros(
            (source_frames,) + tuple(self.smpl_qauts[:, target_id].shape[1:]),
            device=self.smpl_qauts.device,
            dtype=self.smpl_qauts.dtype,
        )
        if valid_frame_idx.numel() > 0:
            motion_smpl_qauts[valid_frame_idx] = smpl_qauts_valid
        return motion_smpl_qauts
    ...
    def _apply_motion_asset(
        ...
        motion_fv, motion_smpl_qauts = self._align_motion_to_target_frames(
            ...
            motion_smpl_qauts=motion_smpl_qauts,
            target_original_fv=target_original_fv,
            preserve_target_fv=preserve_target_fv,
        )
    ...
    def replace_instance_with_ply(
        ...
        new_id: int = None,
        copy_motion: bool = False,
        motion_source: Optional[Union[int, str, bool]] = None,
    ):
        del new_id  # Keep the same signature as RigidNodes for edit-config compatibility.
        motion_source = self._normalize_motion_source(motion_source)
        if motion_source is None and copy_motion:
            motion_source = ply_path

        asset = self._build_gaussian_asset_from_ply(ply_path)
        sidecar_asset, sidecar_path = self._load_sidecar_motion_asset(ply_path)

        motion_asset = self._resolve_motion_asset(motion_source)
        if motion_asset is None and motion_source == ply_path and sidecar_asset is not None:
            motion_asset = sidecar_asset
    ...
    def add_instance_with_ply(
        ...
        ref_instances_fv, ref_instances_trans, ref_instances_quats, ref_instances_size, ref_desc = self._resolve_reference_source(target_id)
    ...
    def export_instance_to_ply(
        ...
        if self.use_voxel_deformer:
            lbs_voxel_base = self.template.voxel_deformer.lbs_voxel_base[template_instance_id].detach().cpu().numpy().reshape(-1)
            voxel_w_correction = self.template.voxel_deformer.voxel_w_correction[template_instance_id].detach().cpu().numpy().reshape(-1)
            ...
            meta_dtype = [("offset_0", "f4"), ("offset_1", "f4"), ("offset_2", "f4"), ("scale_0", "f4")]
            meta_arr = np.empty(1, dtype=meta_dtype)
