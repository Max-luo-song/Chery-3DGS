...
class RoadNodes(nn.Module):
    ...
    def _init_empty(self) -> None:
        self._means = Parameter(torch.empty(0, 3, device=self.device))
        scale_dim = 1 if self.ball_gaussians else (2 if self.gaussian_2d else 3)
        self._scales = Parameter(torch.empty(0, scale_dim, device=self.device))
        self._quats = Parameter(torch.empty(0, 4, device=self.device))
        self._opacities = Parameter(torch.empty(0, 1, device=self.device))
        self._features_dc = Parameter(torch.empty(0, 3, device=self.device))
        self._features_rest = Parameter(torch.empty(0, num_sh_bases(self.sh_degree) - 1, 3, device=self.device))
        self.filter_mask = torch.empty(0, device=self.device, dtype=torch.bool)
    ...
    def create_from_pcd(self, init_means: torch.Tensor, init_colors: torch.Tensor) -> None:
        ...
        if init_means.shape[0] < k:
            logger.warning(
                f"RoadNodes received only {init_means.shape[0]} init points; disabling RoadNodes for this view group."
            )
            self._init_empty()
            return
        ...
        if len(resampled_means_list) == 0:
            logger.warning("RoadNodes grid resampling produced no points; disabling RoadNodes for this view group.")
            self._init_empty()
            return
