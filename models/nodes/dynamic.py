...
from models.modules import ColorConditionalDeformNetwork
from models.gaussians.basics import *
from models.nodes.rigid import RigidNodes

logger = logging.getLogger()

class TrafficLightNodes(RigidNodes):
    def __init__(
        self,
        **kwargs
    ):
        ...
        self.deform_network = ColorConditionalDeformNetwork(
    ...
    def get_deformation(self, local_means) -> Tuple:
        ...
        delta_xyz, delta_quat, delta_scale, delta_color = self.deform_network(x, t, nonrigid_embed)
        return delta_xyz, delta_quat, delta_scale, delta_color
    
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        filter_mask = torch.ones_like(self._means[:, 0], dtype=torch.bool)
        self.filter_mask = filter_mask
        
        delta_xyz, delta_quat, delta_scale, delta_color = None, None, None, None
        if self.ctrl_cfg.use_deformgs_for_nonrigid and self.step > self.ctrl_cfg.use_deformgs_after:
            delta_xyz, delta_quat, delta_scale, delta_color = self.get_deformation(local_means=self._means)
        ...
        self._gs_cache = {
            ...
            "colors": colors[filter_mask] if delta_color is not None else None,
        }
        return gs_dict
    ...
    def state_dict(self) -> Dict:
        ...
        state_dict["instances_embedding"] = self.instances_embedding
        return state_dict
