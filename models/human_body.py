...
class SMPLTemplate(nn.Module):
    ...
    def forward(self, masked_theta=None, xyz_canonical=None, instances_mask=None):
        ...
        if instances_mask is not None:
            expected_instances = int(instances_mask.shape[0])
            if self.A0_inv.shape[0] != expected_instances or self.J_canonical.shape[0] != expected_instances or self.W.shape[0] != expected_instances:
                raise RuntimeError(
                    "SMPLTemplate buffers are out of sync with node instances: "
                    f"mask={expected_instances}, A0_inv={self.A0_inv.shape[0]}, "
                    f"J_canonical={self.J_canonical.shape[0]}, W={self.W.shape[0]}"
                )
    ...
    def add_instance(self, ins_id, new_dict):
        new_J_canonical = new_dict['J_canonical'][None]
        _, new_A0 = batch_rigid_transform(
            self.canonical_pose[None].to(new_J_canonical),
            new_J_canonical,
            self._template_layer.parents,
        )
        new_A0_inv = torch.inverse(new_A0)
        self._buffers["A0_inv"] = torch.cat([self.A0_inv, new_A0_inv], dim=0)
        self._buffers["J_canonical"] = torch.cat([self.J_canonical, new_dict['J_canonical'][None]], dim=0)
        self._buffers["W"] = torch.cat([self.W, new_dict['W'][None]], dim=0)
        self.num_human += 1
