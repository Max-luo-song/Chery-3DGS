...
def renderComposite(
    ...
    for model_info in valid_model_info:
        ...
        curr_points_with_model_id = torch.full(
            (model_xyz.shape[0],), model_info.model_id
        )
...
def render(
    ...
    visible_mask = prefilter_voxel(viewpoint_cam, gs, pipe, background, max_depth)
    ...
    if isinstance(insert_objs, list) and len(insert_objs) > 0:
        for each_obj in insert_objs:
            ...
            rot_obj2world = rotation_matrix_to_quaternion(
                each_obj["sim_pose"][:3, :3]
