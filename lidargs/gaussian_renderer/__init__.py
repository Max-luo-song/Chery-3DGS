...
from diff_lidargs_surfel_rasterization import (
...
def generate_neural_gaussians(
    ...
    if is_trick:
        ob_view = (
            anchor - viewpoint_camera.lidar_center
        )  # viewpoint_camera.original_lidar_center if viewpoint_camera.original_lidar_center is not None else anchor - viewpoint_camera.lidar_center
    else:
        ob_view = (
            anchor - viewpoint_camera.camera_center
        )  # 对于obj来说 camera_center 与lidar center不同 这是个bug 暂时将错就错

    # dist
    ob_dist = ob_view.norm(dim=1, keepdim=True).clamp_min(1e-6)
    # view
    ...
    color = color.reshape(
        [anchor.shape[0] * pc.n_offsets, pc.color_channel - 1]
    )  # TODO if using new raydrop mlp, should pc.color_channel-1
    raydrop = raydrop.reshape([anchor.shape[0] * pc.n_offsets, 1])
    color = torch.cat(
        [color, raydrop], dim=1
    )  # color and raydrop will be concatenated together , as the rasterization' input
    ...
    scaling_repeat, repeat_anchor, color, scale_rot, offsets = masked.split(
        [5, 3, pc.color_channel, 6, 3], dim=-1
    )
    # post-process cov
...
def renderComposite(
    ...
    insert_objs=None,
    retain_grad=True,
    composite_insert_render=False,
    *,
    lidar_hfov=None,
    use_multi_stream=False,
):
    # hfov 是传感器几何，随相机走；调用方不传时从相机读（与 beam_inclinations 一致）
    if lidar_hfov is None:
        lidar_hfov = viewpoint_cam.lidar_hfov
    total_opacity_dict = {}
    total_mask_dict = {}
    total_visable_dict = {}
    total_xyz = None
    ...
    for model_info in valid_model_info:
        ...
        model_visible_mask = prefilter_voxel(
            ...
            use_multi_stream=use_multi_stream,
        )
        ...
        curr_screenspace_points = torch.zeros(
            (model_xyz.shape[0], 4),
            dtype=data_type,
            requires_grad=retain_grad,
            device="cuda",
        )
        curr_points_with_model_id = torch.full(
            (model_xyz.shape[0],),
            model_info.model_id,
            dtype=torch.long,
            device="cuda",
        ...
        model_to_world_r = model_info.model_world_rotation
        model_to_world_t = model_info.model_world_translation
        model_to_world_q = model_info.model_world_quaternion

        w1 = model_to_world_q[0]
        x1 = model_to_world_q[1]
        y1 = model_to_world_q[2]
        z1 = model_to_world_q[3]
        ...
        if not init:
            ...
            total_screenspace_points = torch.cat(
                (total_screenspace_points, curr_screenspace_points), dim=0
            )
            total_points_with_model_id = torch.cat(
                (total_points_with_model_id, curr_points_with_model_id), dim=0
            )
    ...
    if isinstance(insert_objs, list) and len(insert_objs) > 0:
        for each_obj in insert_objs:
            assert each_obj.get("sim_pose") is not None, \
                "insert_objs 中每个 obj 必须包含有效的 sim_pose 字段"
            ...
            total_scaling = torch.cat(
                (total_scaling, each_obj["scaling"][:, :2]), dim=0
            )
            ...
            obj_screenspace_points = torch.zeros(
                (each_obj["xyz"].shape[0], 4),
                dtype=data_type,
                requires_grad=False,
                device="cuda",
            )
            total_screenspace_points = torch.cat(
                (total_screenspace_points, obj_screenspace_points), dim=0
            )
...
def render(
    viewpoint_cam, background, pipe, gs, max_depth, insert_objs=None, retain_grad=False,
    *, lidar_hfov=None, use_multi_stream=False
):
    """
    Render the scene.
    ...
    visible_mask = prefilter_voxel(viewpoint_cam, gs, pipe, background, max_depth, lidar_hfov=lidar_hfov, use_multi_stream=use_multi_stream)

    xyz, color, opacity, scaling, rot, neural_opacity, mask = generate_neural_gaussians(
        viewpoint_cam, gs, visible_mask, is_training=True, is_trick=True
    )

    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    screenspace_points = (
        torch.zeros(
            (xyz.shape[0], 4), dtype=xyz.dtype, requires_grad=is_training, device="cuda"
        )
        + 0
    )
    if retain_grad:
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        campos=viewpoint_cam.lidar_center,
        prefiltered=False,
        beam_inclinations=viewpoint_cam.beam_inclinations,
        hfov=lidar_hfov,
        debug=pipe.debug,
        lidar_far=int(max_depth),
        lidar_near=int(0),
    ...
    if isinstance(insert_objs, list) and len(insert_objs) > 0:
        for each_obj in insert_objs:
            ...
            # 如果植入的物体不去转它，这个分量其实就是单位阵，但是需要考虑植入时去转它的情况，这时候世界坐标系下的rot会变
            rot_obj2world = rotation_matrix_to_quaternion(each_obj["sim_pose"][:3, :3])
            real_rot = quaternionRawMultiply(each_obj["rot"], rot_obj2world)
            ...
            screenspace_points = torch.cat(
                (screenspace_points, obj_screenspace_points), dim=0
            )
        rendered_image_2, radii_2, allmap_2, _ = rasterizer(
            means3D=xyz,
            means2D=screenspace_points,
...
def prefilter_voxel(
    ...
    *,
    ...
    """
    if lidar_hfov is None:
        lidar_hfov = viewpoint_camera.lidar_hfov
    raster_settings = GaussianRasterizationSettings(
