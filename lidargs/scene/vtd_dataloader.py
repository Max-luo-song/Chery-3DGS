...
class Vtd_Dataloader:
    """
    Load Vtd dataset, output as Waymo format
    """
    # ========== 垂直方向参数 ==========
    FOV_UP = 15.0 * torch.pi / 180.0  # 向上15度
    FOV_DOWN = 15.0 * torch.pi / 180.0  # 向下15度（总共30度）
    NUM_BEAMS = 100  # 100线
    H_LIDAR = 100  # 垂直分辨率
    VERTICAL_ANGLE_RESOLUTION = 0.3  # 垂直角分辨率（度）
    
    # ========== 水平方向参数 ==========
    HORIZONTAL_ANGLE_RESOLUTION = 0.2  # 水平角分辨率（度）
    # 前向扫描模式
    FOV_HORIZONTAL_FRONT = 120 * torch.pi / 180.0  # 120度
    W_LIDAR_FRONT = 600  # 水平分辨率
    
    # 默认使用前向扫描模式（可根据需要切换）
    FOV_HORIZONTAL = FOV_HORIZONTAL_FRONT
    W_LIDAR = W_LIDAR_FRONT
    
    # 生成垂直方向的beam inclinations（均匀分布）
    # 从-15度到+15度，共100线，均匀分布
    ELEVATION = np.linspace(-15.0, 15.0, NUM_BEAMS)  # 均匀分布
    BEAM_INCLINATIONS = np.sort(ELEVATION) * np.pi / 180  # 从小到大排列，弧度制
