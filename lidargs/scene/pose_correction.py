...
class PoseCorrection(torch.nn.Module):
    ...
    def training_setup(self):
        ...
        params = [
            {
                "params": [self.pose_correction_trans],
                "lr": pose_correction_lr_init,
                "name": "pose_correction_trans",
            },
            {
                "params": [self.pose_correction_rots],
                "lr": pose_correction_lr_init,
                "name": "pose_correction_rots",
            },
        ]
        self.optimizer = torch.optim.Adam(
            params=params, lr=0, eps=1e-8, weight_decay=0.01
        )
        self.pose_correction_scheduler_args = get_expon_lr_func(
            lr_init=pose_correction_lr_init, lr_final=pose_correction_lr_final
        )
    ...
    def forward(self, cam_id):
        ...
        pose_correction_rot = torch.nn.functional.normalize(
            pose_correction_rot.unsqueeze(0)
        )
        pose_correction_rot = build_rotation(pose_correction_rot).squeeze(0)
        pose_correction_matrix = torch.cat(
            [pose_correction_rot, pose_correction_trans[:, None]], dim=-1
        )
    ...
    def correct_gaussian_xyz(self, cam_id, xyz):
        ...
        pose_correction_rot = self.pose_correction_rots[cam_id]
        pose_correction_rot = torch.nn.functional.normalize(
            pose_correction_rot.unsqueeze(0), dim=-1
        )
        pose_correction_rot = build_rotation(pose_correction_rot).squeeze(0)
        pose_correction_matrix = torch.cat(
            [pose_correction_rot, pose_correction_trans[:, None]], dim=-1
    ...
    def regularization_loss(self):
        ...
        loss_rots = torch.abs(
            rots_norm - torch.tensor([[1, 0, 0, 0]]).float().cuda()
        ).mean()
