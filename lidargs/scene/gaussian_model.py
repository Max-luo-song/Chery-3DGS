...
class GaussianModel(torch.nn.Module):  #
    ...
    def __init__(
        ...
        # 多分辨率feature, 输入[ob_view, ob_dist]，输出粗、中、细粒度的权重
        ...
        # 预测协方差，可选择是否加入距离作为输入，上三角或者下三角，6个值
        self.add_cov_dist = add_cov_dist
    ...
    def capture(self):
        return {
            "active_sh_degree": getattr(self, "active_sh_degree", 0),
            "anchor": self._anchor,
            "offset": self._offset,
    ...
    def restore(self, model_args, training_args):
        if isinstance(model_args, dict):
            ...
            anchor = model_args["anchor"]
            offset = model_args["offset"]
            anchor_feat = model_args["anchor_feat"]
            scaling = model_args["scaling"]
            rotation = model_args["rotation"]
            opacity = model_args["opacity"]
            self.max_radii2D = model_args["max_radii2D"]
            denom = model_args.get("anchor_demon")
            opt_dict = model_args.get("optimizer")
            self.spatial_lr_scale = model_args.get("spatial_lr_scale", 0)
        else:
            (
        ...
        self.anchor_demon = (
            ...
            else torch.zeros((self._anchor.shape[0], 1), device="cuda")
        )
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # start : stop : step, step = ratio, 下采样
    ...
    def anchor_growing(self, grads, threshold, offset_mask):
        ...
        for i in range(self.update_depth):
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                new_feat_max = torch.full(
                    (selected_grid_coords_unique.shape[0], new_feat.size(1)),
                    torch.finfo(new_feat.dtype).min,
                    dtype=new_feat.dtype,
                    device=new_feat.device,
                )
                new_feat_max.scatter_reduce_(
                    0,
                    inverse_indices.unsqueeze(1).expand_as(new_feat),
                    new_feat,
                    reduce="amax",
                    include_self=True,
                )
                new_feat = new_feat_max[remove_duplicates]
