...
from utils.data_partition_utils import dataPartition, getBlockInfo, dataPartitionSimple, dataPartitionChery
...
def training(gt_dynamic_model, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if torch.isnan(loss).any() or torch.isinf(loss).any():
            ...
            print(f"Loss components: depth={depth_loss.item()}, scaling_reg={scaling_reg.item()}, intensity={intensity_loss.item()}, grad={grad_loss.item()}")
            # 打印 scaling 的统计信息
            # 保存当前模型状态用于调试
            torch.save(model_gaussian.capture(), f"debug_model_nan_{iteration}.pth")
        ...
        for name, param in model_gaussian.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                    print(f"[WARNING] NaN/Inf found in gradient of {name} at iteration {iteration}")
        ...
        with torch.no_grad():
            ...
            if iteration in testing_iterations:
                ...
                lidar_to_world_start = np.loadtxt(
                    os.path.join(lidarpose_filefolder, str(start_frame).zfill(3)+'.txt')
                )
                print("---------------------train----------------------")
                train_composite_report(tb_writer, dataset, dataset_name, iteration, train_views, test_timestamp,\
                    model_id_scene_info, valid_timestamp_model, pipe, background, logger, lidar_to_world_start)
...
if __name__ == "__main__":
    ...
    if "chery" in model_path or "orinY" in model_path:
        block_info_with_extend, block_info_without_extend = dataPartitionChery(model_args, single_block_test=True)
    else:
