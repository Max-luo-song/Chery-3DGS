...
def kalman_filter(
    ...
    ax, ay = 0, 0
    x = np.array(
        [float(observations[0][0]), float(observations[0][1]), vx, vy, ax, ay]
    ).reshape(6, 1)
    P = np.eye(6)  # 初始状态协方差
    ...
    for idx, observation in enumerate(observations):
        ...
        if idx != len(observations) - 1:
            ...
            trans_velocities.append(
                [diffx / time_intervels[idx + 1], diffy / time_intervels[idx + 1]]
            )
    if vis:
        import matplotlib.pyplot as plt
        ...
        pred_vel = np.sqrt(
            np.power(predicted_velocities_x, 2) + np.power(predicted_velocities_y, 2)
        )
        trans_velocities_x = np.array(trans_velocities)[:, 0]
        trans_velocities_y = np.array(trans_velocities)[:, 1]
        trans_vel = np.sqrt(
            np.power(trans_velocities_x, 2) + np.power(trans_velocities_y, 2)
        )
        plt.figure()
        plt.plot(
            range(len(predicted_velocities_x)),
            predicted_velocities_x,
            c="r",
            label="pred_vx",
        )
        plt.plot(
            range(len(trans_velocities_x)), trans_velocities_x, c="b", label="trans_vx"
        )

        plt.plot(
            range(len(predicted_velocities_y)),
            predicted_velocities_y,
            c="y",
            label="pred_vy",
        )
        plt.plot(
            range(len(trans_velocities_y)), trans_velocities_y, c="g", label="trans_vy"
        )
        plt.legend()
        plt.show()
        plt.savefig("./tmp_vis_kmfilter_velocity.png")
    return np.array(predicted_positions).squeeze(-1), np.array(
        predicted_velocities
    ).squeeze(-1)
...
def generate_vel_gt(
    ...
    for frame_id, frame_info in enumerate(data[seq]["seq_info"]):
        ...
        instances[gt_boxe_token].append(
            {
    ...
    for token_name, instance in instances.items():
        ...
        for frame_id in range(len(data[seq]["seq_info"])):
            gt_boxes_velocity_datas = data[seq]["seq_info"][frame_id][
                "gt_boxes_velocity"
            ]
            new_gt_boxes_velocity_datas = np.concatenate(
                (
                    gt_boxes_velocity_datas,
                    np.array(
                        [[pred_vel[frame_id][0], pred_vel[frame_id][1]]],
                        dtype=np.float32,
                    ),
                ),
                axis=0,
            ...
            if frame_id == 0:
                print(new_gt_boxes_velocity_datas.shape)
            data[seq]["seq_info"][frame_id][
                "gt_boxes_velocity"
            ] = new_gt_boxes_velocity_datas
